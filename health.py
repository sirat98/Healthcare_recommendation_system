...
# Symptom & disease dictionaries (place your full version here)
...
def helper(disease):
    ...
    pre = [p for p in pre.values[0]] if not pre.empty else []
...
# Streamlit UI
st.set_page_config(page_title="🩺 Medicine Recommender")
st.title(" Disease & Medicine Recommendation System")
...
if st.button("Predict"):
    if user_input.strip():
        ...
        predicted = get_predicted_value(symptoms)
        desc, pre, meds, diet, workout_plan = helper(predicted)
        ...
        for i, w in enumerate(workout_plan, 1):
            st.markdown(f"{i}. {w}")
